    ...
    saveMethod(){
        ...
        if(z.firstElementChild){
            for(let i = 0; i <localStorage.length; i++) {
                let keyName=  document.querySelector('.tab_menu .list .is_on .btn').innerText
            ...
            if (keyName !== null) 
            {
                ...
                if(keyName !=='')
                {
                    for(let i = 0; i <localStorage.length; i++)
                    {
                        ...
                        {
                            alertV=1;
                            alert(keyName+" 파일이 이미 존재합니다. 다른 이름으로 저장하세요.")
                            break
                        }
                    ...
                    if(alertV==0)
                    {
                        ...
                        localStorage.setItem(keyName, data)                 
                        document.querySelector('.tab_menu .list .is_on .btn .fileName').innerText=keyName
                        indicator.style.display='none'
                    }
            ...
            if (a !== null) {
                for(let i = 0; i <localStorage.length; i++) {
                    if(localStorage.key(i)==a){             
                        ...
                        input.value = data2;
                        document.querySelector('.tab_menu .list .is_on .btn .fileName').innerHTML=a
                        alertV=1
                        input.addEventListener('keydown',()=>this.indicatorMethod())
                        break;          
                    }
