...
app.post("/login", (req, res) => {
  ...
  if(getUser.userpw !== userpw){
    res.send("아이디와 패스워드가 없습니다.");
  }

  const token =jwt.sign(getUser, process.env.MY_SECRET, {expireIn:'1h'}) //토큰 생성
...
app.get("/note/fileContent/:fileName", (req, res) => {
  ...
  fs.readFile(`./file/${req.session.user.userid}/${fileName}.txt`, "utf8", (err, fileData) => {
    if (err) {
      res.end()
    } else {
      res.send(fileData);
...
app.post("/note/delete", (req, res) => {
  fs.access(`./file/${req.session.user.userid}/${req.body.keyName}.txt`, fs.constants.F_OK, (err) => {
    if (err) {
      ...
      fs.unlink(`./file/${req.session.user.userid}/${req.body.keyName}.txt`, (err) => {
        console.log(err);
      });
    }
  });
